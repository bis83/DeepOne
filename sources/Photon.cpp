...
void Photon::Draw() {
	for(int i=0; i<MAX_PHOTON; ++i) {
		if(m_photonlist[i].active) {
			...
			m_pSprTama->SetUV( (m_photonlist[i].type % 4)*32, (m_photonlist[i].type / 4)*32,
				(m_photonlist[i].type % 4)*32+32, (m_photonlist[i].type / 4)*32+32 );
...
int Photon::HitPlayerAndCristal(Player *pPlayer) {
	...
	for(int i=0; i<MAX_PHOTON; ++i) {
		if(m_photonlist[i].active) {
			if(pPlayer->HitPlayer(m_photonlist[i].pos_x, m_photonlist[i].pos_y, 10, m_photonlist[i].type%2)) {
				m_photonlist[i].active = false;
				m_pSeTamakie->Play(false, true);
				ret++;
			}
			if(pPlayer->HitCristal(m_photonlist[i].pos_x, m_photonlist[i].pos_y, 15, m_photonlist[i].type%2)) {
				m_photonlist[i].active = false;
				m_pSeTamakie->Play(false, true);
				ret++;
			}

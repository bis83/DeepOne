			...
			switch(m_gatelist[i].type) {
				...
				case 4:
					if(m_gatelist[i].time % (60 - m_gatelist[i].level%8*3) == 0) {
						...
						for(int j=0; j< (4 + m_gatelist[i].level/10); j++) {
							m_pPhoton->CreateNewPhoton(4, m_gatelist[i].pos_x, m_gatelist[i].pos_y,
								2.0f * cos(2.0f * D3DX_PI * j / (4 + m_gatelist[i].level/10)),
								2.0f * sin(2.0f * D3DX_PI * j / (4 + m_gatelist[i].level/10))
								);
						}
					}
					if((m_gatelist[i].time + 30) % (60 - m_gatelist[i].level%8*3) == 0) {
						...
						for(int j=0; j< (4 + m_gatelist[i].level/10); j++) {
							m_pPhoton->CreateNewPhoton(4, m_gatelist[i].pos_x, m_gatelist[i].pos_y,
								2.0f * cos(2.0f * D3DX_PI * (2*j+1) / ((4 + m_gatelist[i].level/10) * 2) ),
								2.0f * sin(2.0f * D3DX_PI * (2*j+1) / ((4 + m_gatelist[i].level/10) * 2) )
								);
						}
					}
					break;
				case 5:
					if(m_gatelist[i].time % (60 - m_gatelist[i].level%10*3) == 0) {
						...
						for(int j=0, rnd = rand()%360; j< (4 + m_gatelist[i].level/10); j++) {
							...
							m_pPhoton->CreateNewPhoton(5, m_gatelist[i].pos_x, m_gatelist[i].pos_y,
								4.0f * cos(D3DX_PI / 2 * j / (4 + m_gatelist[i].level/10) + 2 * D3DX_PI * rnd / 360),
								4.0f * sin(D3DX_PI / 2 * j / (4 + m_gatelist[i].level/10) + 2 * D3DX_PI * rnd / 360)
								);
						}
					}
					break;
...
int Gate::HitPlayerAndCristal(Player *pPlayer) {
	int ret = 0;
	for(int i=0; i<MAX_GATE; ++i) {
		if(!m_gatelist[i].active)
			continue;
		if(m_gatelist[i].time <= 0)
			continue;
		if(pPlayer->HitCristal(m_gatelist[i].pos_x, m_gatelist[i].pos_y, 32, 0) ||
			pPlayer->HitCristal(m_gatelist[i].pos_x, m_gatelist[i].pos_y, 32, 1)) {
				// ゲートの削除
...
void Gate::CreateNewGate(int type, float pos_x, float pos_y, int level) {
	// レベルによるゲート最大数制限
